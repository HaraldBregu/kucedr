...
The standalone memory module stores `settings.json`, consolidated `MEMORY.md`, and one cumulative Markdown transcript per agent session under `~/.kucedr/memory`. Main, channel, task, health, subagent, and realtime voice runs update `<session-id>.md` atomically. Scheduled processing and **Generate Memory** read these snapshots instead of session storage, validate extracted facts and summaries, and provide bounded relevant context before responses. Memory Settings supports configuration, refresh, forgetting individual entries, and clearing saved memory without displaying the memory document. The `list_memories` and `forget_memory` tools remain thin calls into this module.
