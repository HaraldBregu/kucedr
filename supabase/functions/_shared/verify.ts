...
export async function verify(bucket: string, key: string, expectedHash: string, expectedSize: number) {
  const response = await s3().send(new GetObjectCommand({ Bucket: bucket, Key: key }));
  if (!response.Body || response.ContentLength !== expectedSize) throw new Error('Uploaded object size mismatch');
