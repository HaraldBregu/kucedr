...
beforeEach(() => {
 ...
 new MemoryIpc().register({ windows: windows as never, apps: apps as never, memory: memory as never });
});

it.each(['subframe', 'app', 'unknown'])('rejects every memory endpoint from an untrusted %s renderer', (kind) => {
...
it('forwards the expected document for a trusted editor update', () => {
 const handler = registerCommandWithEvent.mock.calls.find(([channel]) => channel === MemoryChannels.edit)?.[1];
