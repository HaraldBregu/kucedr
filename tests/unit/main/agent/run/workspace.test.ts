...
it.each(['task', 'health', 'child'] as const)(
	'allows workspace commands without a window for %s',
	async (source) => {
		...
		expect(
			(await execute(tool, { command: 'node tools/example.js' }, undefined, source)).at(-1)
...
it('allows workspace bash without approval', async () => {
	...
	expect((await execute(tool, { command: 'node tools/example.js' })).at(-1)).toMatchObject({
		type: 'tool_call_end',
		permissionOutcome: 'allow',
	});
	expect(run).toHaveBeenCalled();
...
it.each(['camera_recorder', 'delegate_a2a'])(
...
it.each(['create_task', 'update_task', 'delete_task'])(
	'allows direct %s requests without approval',
	async (id) => {
		const run = jest.fn().mockResolvedValue('done');
		const tool = jsonTool({ id, name: id, description: id, schema: {}, execute: run });
