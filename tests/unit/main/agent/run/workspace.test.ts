	...
	const directory = jest
		...
		.mkdtempSync(
			jest
				...
				.join(jest.requireActual<typeof os>('node:os').tmpdir(), 'kucedr-workspace-')
		);
...
async function execute(
	...
	history?: FileHistory,
	source?: 'task' | 'health' | 'child'
): Promise<RuntimeEvent[]> {
	const events: RuntimeEvent[] = [];
	const security = source
		? { runId: scope.runId, scope: { ...scope, source } }
		: { runId: scope.runId, windowId: 1, scope };
	for await (const event of runToolCall(
		tool,
		{ id: crypto.randomUUID(), name: tool.id, args },
		undefined,
		undefined,
		security,
		undefined,
...
it.each([undefined, 'task', 'health', 'child'] as const)(
	'creates, reads, overwrites, edits, moves, deletes, undoes and redoes workspace files without approval (source: %s)',
	async (source) => {
		...
		for (const [tool, args] of operations) {
			expect((await execute(tool, args, history, source)).at(-1)).toMatchObject({
				type: 'tool_call_end',
...
it.each(['task', 'health', 'child'] as const)(
	'allows workspace commands and memory updates without a window for %s',
	async (source) => {
		for (const id of ['bash', 'save_memory', 'forget_memory', 'update_health']) {
			...
			expect(
				(
					await execute(
						tool,
						id === 'bash' ? { command: 'node tools/example.js' } : {},
						undefined,
						source
					)
				).at(-1)
			).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'allow' });
...
it.each(['task', 'health', 'child'] as const)(
	'blocks unapproved outside access without a window for %s',
	async (source) => {
		const run = jest.fn();
		const bash = jsonTool({
			id: 'bash',
		...
		for (const [tool, args] of operations) {
			expect((await execute(tool, args, undefined, source)).at(-1)).toMatchObject({
				type: 'tool_call_end',
...
it.each(['save_memory', 'forget_memory', 'update_health', 'bash'])(
	'allows workspace %s without approval',
	async (id) => {
		const run = jest.fn().mockResolvedValue('done');
		const tool = jsonTool({ id, name: id, description: id, schema: {}, execute: run });
		expect(
			(await execute(tool, id === 'bash' ? { command: 'node tools/example.js' } : {})).at(-1)
		).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'allow' });
		expect(run).toHaveBeenCalled();
	}
);
...
it('allows overwriting workspace names beginning with two dots', async () => {
	...
	expect(
		(await execute(writeTool, { path: '..cache/file.txt', content: 'new' })).at(-1)
	).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'allow' });
	expect(fs.readFileSync(path.join(workspace, '..cache/file.txt'), 'utf8')).toBe('new');
});

it.each(
	([undefined, 'task', 'health', 'child'] as const).flatMap((source) =>
		['kill', 'clear', 'remove'].map((action) => [action, source] as const)
	)
)('allows %s for an owned workspace sandbox process (source: %s)', async (action, source) => {
	const session = {
		id: action,
	...
	try {
		...
		expect(
			(
				await execute({ ...processTool, run }, { action, sessionId: session.id }, undefined, source)
			).at(-1)
		).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'allow' });
		expect(run).toHaveBeenCalled();
	} finally {
...
it.each(['../outside.txt', '../workspace-copy/outside.txt'])(
	'asks before writing %s outside the workspace',
	async (target) => {
		expect((await execute(writeTool, { path: target, content: 'outside' })).at(-1)).toMatchObject({
...
it('asks before following a workspace symlink to an outside file', async () => {
	...
	try {
		...
		expect(
			(await execute(writeTool, { path: 'linked/file.txt', content: 'outside' })).at(-1)
		).toMatchObject({ type: 'tool_permission_request' });
...
it.each([
	{ command: 'pwd', additionalRoots: ['..'] },
	{ command: 'pwd', elevated: true },
])('asks before command access beyond the workspace: %j', async (args) => {
	const run = jest.fn();
	const tool = jsonTool({
		id: 'bash',
		name: 'bash',
...
it('keeps explicit workspace deny rules effective', async () => {
	...
	setPermissions({
		...permissions,
		write: { ...permissions.write, deny: [path.join(workspace, 'blocked.txt')] },
	});
	expect(
		(await execute(writeTool, { path: 'blocked.txt', content: 'blocked' })).at(-1)
	).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'deny' });
	expect(fs.existsSync(path.join(workspace, 'blocked.txt'))).toBe(false);
});

it.each(['camera_recorder', 'open_apps', 'create_task'])(
	'retains approval for %s effects beyond workspace files',
	async (id) => {
		const run = jest.fn();
		const tool = jsonTool({ id, name: id, description: id, schema: {}, execute: run });
...
it.each(['microphone_recorder', 'screen_recorder'])(
	'allows %s in the workspace without forced approval',
	async (id) => {
		const run = jest.fn().mockResolvedValue('started');
		const tool = jsonTool({
			id,
			name: id,
			description: id,
...
it('reads outside files and runs sandboxed commands from outside without approval', async () => {
	...
	expect((await execute(readTool, { path: target })).at(-1)).toMatchObject({
		type: 'tool_call_end',
		permissionOutcome: 'allow',
	...
	expect(
		(await execute(bash, { command: 'cat outside-read.txt', workdir: '..' })).at(-1)
	).toMatchObject({ type: 'tool_call_end', permissionOutcome: 'allow' });
	expect(run).toHaveBeenCalledTimes(1);
});

it.each(['approve', 'approve_always', 'reject'] as const)(
	'records %s for an outside overwrite and reuses only a saved grant',
	async (decision) => {
		...
		const events = runToolCall(
			writeTool,
			{ id: decision, name: 'write', args: { path: target, content: 'changed' } },
			undefined,
			undefined,
			{ runId: scope.runId, windowId: 1 }
		);
		await events.next();
		...
		expect(request).toMatchObject({
			type: 'tool_permission_request',
			persistable: true,
			targets: [realPath(path.dirname(target))],
		});
		...
		expect(JSON.parse(stringifyRunEntry(outcome)!)).toMatchObject({
			event: { toolCallId: decision, permissionOutcome: decision },
		});
		await events.next();
		expect(fs.readFileSync(target, 'utf8')).toBe(decision === 'reject' ? 'original' : 'changed');
		expect(getPermissions().write.allow.includes(`${realPath(path.dirname(target))}/**`)).toBe(
			decision === 'approve_always'
		);
		const next = await execute(writeTool, { path: target, content: 'again' });
		expect(next.at(-1)?.type).toBe(
			decision === 'approve_always' ? 'tool_call_end' : 'tool_permission_request'
		);
	}
);
...
it('reuses a saved outside location for create, edit, move, delete, undo and redo across runs', async () => {
	...
	setPermissions({
		...
		write: {
			...permissions.write,
			allow: [...permissions.write.allow, `${realPath(directory)}/**`],
		},
	});
	...
	for (const [tool, args] of operations) {
		expect((await execute(tool, args, history, 'child')).at(-1)).toMatchObject({
			type: 'tool_call_end',
			permissionOutcome: 'allow',
			isError: undefined,
		});
...
it('offers a reusable folder grant for undo outside the workspace', async () => {
	...
	setPermissions({
		...permissions,
		write: { ...permissions.write, allow: [...permissions.write.allow, realPath(target)] },
	});
	await execute(writeTool, { path: target, content: 'created' }, history);
	resetPermissions();
	expect((await execute(undoFileTool(history), {}, history)).at(-1)).toMatchObject({
		type: 'tool_permission_request',
		persistable: true,
		targets: [realPath(path.dirname(target))],
	});
...
it('persists an outside command grant and reuses it without an approval window', async () => {
	...
	const events = runToolCall(
		bash,
		{ id: 'command-grant', name: 'bash', args },
		undefined,
		undefined,
		{ runId: scope.runId, windowId: 1 }
	);
	...
	expect((await end).value).toMatchObject({
		type: 'tool_call_end',
		permissionOutcome: 'approve_always',
	});
	await events.next();
	expect(getPermissions().exec.allow).toContain(`${realPath(directory)}/**`);
	expect((await execute(bash, args, undefined, 'child')).at(-1)).toMatchObject({
		type: 'tool_call_end',
		permissionOutcome: 'allow',
	});
