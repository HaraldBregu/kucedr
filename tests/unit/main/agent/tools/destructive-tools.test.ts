	...
	const directory = jest
		...
		.mkdtempSync(
			jest
				...
				.join(jest.requireActual<typeof os>('node:os').tmpdir(), 'kucedr-destructive-')
		);
...
it.each(['overwrite', 'delete', 'move'])(
	'requests a reusable location grant before outside %s',
	async (operation) => {
		resetPermissions();
		...
		const events = runToolCall(tool, { id: operation, name: tool.id, args }, undefined, undefined, {
			runId: 'run',
			windowId: 1,
		});
		await events.next();
		try {
			expect((await events.next()).value).toMatchObject({
				...
				persistable: true,
				targets: [realPath(userDataLocation())],
