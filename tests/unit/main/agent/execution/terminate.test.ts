...
unix(
	'escalates termination for a process group that ignores SIGTERM',
	async () => {
		const child = spawn('/bin/sh', ['-c', 'trap "" TERM; echo ready; sleep 30'], {
			detached: true,
		});
		try {
...
unix(
	'retains cancellation ownership after a shell exits with a live descendant',
	async () => {
