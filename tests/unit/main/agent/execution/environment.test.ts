...
it('does not inherit application credentials into shell commands', () => {
	expect(
		commandEnvironment(
			{},
			{ PATH: '/bin', LANG: 'en_US.UTF-8', API_KEY: 'private', NODE_OPTIONS: '--require=payload' }
		)
...
])('rejects command overrides of %s before helper startup', (name) => {
...
it('supports explicitly supplied command environment values', () => {
	expect(commandEnvironment({ BUILD_MODE: 'test' }, { PATH: '/bin' })).toEqual({
		PATH: '/bin',
		BUILD_MODE: 'test',
	});
});
...
it('rejects Plan environment overrides before launching its sandbox helper', () => {
	expect(planCommandError({ command: 'pwd', env: { PATH: '/untrusted' } }, '/workspace')).toContain(
		'helper environment'
	);
});
