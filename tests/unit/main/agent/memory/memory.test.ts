...
function setup(initialized = true) {
	...
	const sessions: SourceSession[] = [
		{
			id: 'chat',
			messages: [{ fingerprint: 'first', role: 'user', text: 'I prefer concise answers.' }],
		},
	];
	...
	const dependencies: MemoryDependencies = {
		...
		sources: jest.fn(async () => structuredClone(sessions)),
		exists,
		read: async () => markdown,
		write,
		writeSession,
...
it.each(['provider', 'malformed', 'write'])(
	'keeps checkpoints pending after a %s failure',
	async (failure) => {
		const h = setup();
...
it('keeps memory model configuration independent from the chat selection', async () => {
	...
	await h.memory.refresh();
	expect(h.memory.getConfig()).toMatchObject({
		providerId: 'provider',
		modelId: 'independent',
...
it('initializes the memory model from chat only once', async () => {
	...
	await memory.stop();
	(h.dependencies.selection as jest.Mock).mockReturnValue({
...
it.each(['My password is secret123.', 'My medical record is private.'])(
	'excludes private source content: %s',
	async (text) => {
		const h = setup();
...
it('rejects private output and assistant-only sources', async () => {
	...
	h.sessions[0].messages = [
		{ fingerprint: 'assistant', role: 'assistant', text: 'The user likes Go.' },
	];
	await h.memory.refresh();
	expect(h.infer).toHaveBeenCalledTimes(1);
});
...
it.each([true, false])(
	'recovers a mutation journal only when its document was committed: %s',
	async (committed) => {
		const h = setup();
...
it('rejects stale manual edits without changing memory or checkpoints', async () => {
	const h = setup();
	await expect(h.memory.edit('replacement', 'stale document')).rejects.toThrow(
		'Memory changed while editing'
	);
	expect(h.write).not.toHaveBeenCalled();
	expect(h.state().checkpoints).toEqual({});
...
it('summarizes a new assistant reply using preceding user context without re-extracting old facts', async () => {
	...
	h.sessions[0].messages.push({
		fingerprint: 'answer',
		role: 'assistant',
		text: 'We agreed to keep future answers concise and focused.',
	});
...
it('merges concurrent configuration patches instead of losing either update', async () => {
	const h = setup();
	await Promise.all([
		h.memory.configure({ enabled: false }),
		h.memory.configure({ memoryType: 'facts' }),
	]);
