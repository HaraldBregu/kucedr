...
jest.mock('electron-store', () => {
	...
	return class {
		...
		get store(): Record<string, unknown> { return this.value; }
...
it('copies existing cloud settings into storage/settings.json and removes the app copy', () => {
	const legacy = {
		...
		paths: ['/data/projects'], syncEnabled: true, syncCronExpression: '0 4 * * *',
	};
	...
	jest.isolateModules(() => {
		const settings = require('../../../../src/main/settings_store') as typeof import('../../../../src/main/settings_store');
		expect(settings.getStorageSettings()).toEqual(legacy);
	});

	expect(JSON.parse(readFileSync(path.join(root, 'storage', 'settings.json'), 'utf8'))).toEqual(legacy);
...
it('keeps an existing dedicated store authoritative during migration', () => {
	...
	writeFileSync(appFile, JSON.stringify({ cloud: {
		paths: ['/data/old'], syncEnabled: false, syncCronExpression: '0 3 * * *',
	} }));
	const selected = { paths: ['/data/current'], syncEnabled: false, syncCronExpression: '0 4 * * *' };
