...
const originalId = process.env.MCP_GOOGLE_CLIENT_ID;
const originalSecret = process.env.MCP_GOOGLE_CLIENT_SECRET;

afterEach(() => {
	if (originalId === undefined) delete process.env.MCP_GOOGLE_CLIENT_ID;
	else process.env.MCP_GOOGLE_CLIENT_ID = originalId;
	if (originalSecret === undefined) delete process.env.MCP_GOOGLE_CLIENT_SECRET;
	else process.env.MCP_GOOGLE_CLIENT_SECRET = originalSecret;
});
...
it.each(['gmailmcp', 'calendarmcp', 'drivemcp'])(
	...
	(host) => {
		process.env.MCP_GOOGLE_CLIENT_ID = 'environment-id';
		process.env.MCP_GOOGLE_CLIENT_SECRET = 'environment-secret';
