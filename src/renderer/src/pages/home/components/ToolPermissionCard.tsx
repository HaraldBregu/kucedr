...
export function ToolPermissionCard({
	...
	const input =
		permission.input && typeof permission.input === 'object'
		...
		<Card size="sm" className="mb-2 w-full max-w-2xl" role="alert" aria-live="assertive">
			<CardHeader>
				...
				<CardAction>
					<Badge variant={hostExecution || destructiveOperation ? 'destructive' : 'secondary'}>
						{sensitiveOperation
							? t('toolPermission.sensitiveAccess', { defaultValue: 'Sensitive action' })
							: t(badgeKey)}
					</Badge>
				</CardAction>
			</CardHeader>
			<CardContent className="space-y-2">
				...
				<p className="text-xs text-muted-foreground">
					{sensitiveOperation
						? t('toolPermission.sensitiveReason', {
								defaultValue:
									'Approve this specific action before the assistant accesses a service, records media, spends money, or saves lasting changes.',
							})
						: t(reasonKey)}
				</p>
				{detail && (
					<pre className="max-h-24 overflow-auto whitespace-pre-wrap break-all rounded-md bg-muted px-2 py-1.5 font-mono text-xs text-muted-foreground">
						<code>{detail}</code>
					</pre>
				)}
				{error && (
			...
			<CardFooter className="flex flex-wrap justify-end gap-2">
				...
				{permission.persistable && (
					...
					>
						{t('toolPermission.trustLocation')}
					</Button>
				)}
				{permission.allowOnce && (
					<Button type="button" size="sm" disabled={responding} onClick={() => respond('approve')}>
						{t('toolPermission.allowOnce')}
					</Button>
				)}
			</CardFooter>
		</Card>
