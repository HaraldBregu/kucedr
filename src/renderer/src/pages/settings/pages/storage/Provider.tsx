...
export default function Provider({ providers, providerId, disabled, onChange }: ProviderProps): React.JSX.Element {
		...
		<SettingsPanel>
			<SettingsRow
				...
				description={selected
					...
					: t(providers.length === 0 ? 'settings.storage.provider.empty' : providerId
						? 'settings.storage.provider.missing' : 'settings.storage.provider.description')}
				actions={
					<div className="flex w-full min-w-0 flex-col items-start gap-2 sm:w-auto sm:items-end">
						...
						>
							<SelectTrigger className="w-full max-w-full text-xs sm:w-64" aria-label={t('settings.storage.provider.title')}>
						...
						<Link to="/settings/providers/storage" className={buttonVariants({ variant: 'link', size: 'xs' })}>
