...
export default function StorageForm({ provider, onSaved, onCancel }: StorageFormProps): React.JSX.Element {
	...
	const canSave = ['name', 'bucket', 'region', 'accessKeyId'].every((key) =>
	...
	return (
		<Dialog open onOpenChange={(open) => { if (!open && !saving) onCancel(); }}>
			<DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-lg" showCloseButton={!saving}>
				<DialogHeader>
					<DialogTitle>{t(provider ? 'settings.storageProviders.editTitle' : 'settings.storageProviders.add')}</DialogTitle>
				...
				<form className="grid gap-4" onSubmit={(event) => {
					...
					setSaving(true);
					setError('');
					void window.storage.saveProvider(draft).then(onSaved).catch((err: unknown) => {
					...
					<fieldset disabled={saving} className="grid min-w-0 gap-4 sm:grid-cols-2">
						{FIELDS.map((field) => {
							const description = field.key === 'endpoint' ? t('settings.storageProviders.endpointHint')
								: field.key === 'secretAccessKey' && provider?.hasSecretAccessKey
									? t('settings.storageProviders.secretHint') : undefined;
							return <SettingsField
								key={field.key} id={`storage-${field.key}`}
								label={t(`settings.storageProviders.fields.${field.key}`)} description={description}
								className={field.key === 'endpoint' || field.key === 'name' ? 'sm:col-span-2' : undefined}
							>
								<Input
									id={`storage-${field.key}`} type={field.type} autoComplete="off" spellCheck={false}
									aria-describedby={description ? `storage-${field.key}-description` : undefined}
									required={field.key !== 'endpoint' && !(field.key === 'secretAccessKey' && provider?.hasSecretAccessKey)}
									placeholder={field.placeholder} value={draft[field.key] ?? ''}
									onChange={(event) => setDraft((current) => ({ ...current, [field.key]: event.target.value }))}
								/>
							</SettingsField>;
						})}
						<SettingsField id="storage-path-style" label={t('settings.storageProviders.fields.forcePathStyle')}>
							<Switch id="storage-path-style" checked={draft.forcePathStyle}
								onCheckedChange={(checked) => setDraft((current) => ({ ...current, forcePathStyle: checked }))} />
						</SettingsField>
					</fieldset>
					<div className="flex justify-end gap-2">
						<Button type="button" variant="outline" disabled={saving} onClick={onCancel}>{t('common.cancel')}</Button>
