	...
	return (
		<SettingsPageShell>
			<SettingsPageHeader
				...
				action={
					<Button size="sm" disabled={loading || removing !== null} onClick={() => setEditing('new')}>
			...
			<SettingsPanel>
				{loading ? <SettingsLoadingRows /> : providers.length === 0 ? (
					!error && <SettingsEmptyState
						icon={HardDrive}
					...
					<SettingsRow
						key={provider.id}
						icon={HardDrive}
						title={<span className="block truncate" title={provider.name}>{provider.name}</span>}
						description={
						...
						actions={
							<>
								<Button
									variant="ghost" size="icon-sm" disabled={removing !== null}
								...
								<Button
									...
									onClick={() => {
										...
										setError('');
										void window.storage.removeProvider(provider.id).then(() => {
											setProviders((current) => current.filter((entry) => entry.id !== provider.id));
										}).catch((err: unknown) => {
											setError(getErrorMessage(err, t('settings.storageProviders.removeError')));
			...
			{editing !== null && <StorageForm
				provider={editing === 'new' ? undefined : editing}
				onCancel={() => setEditing(null)}
				onSaved={(saved) => {
					setProviders((current) => current.some((entry) => entry.id === saved.id)
