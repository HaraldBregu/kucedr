...
export async function scanSources(location: string): Promise<SourceSession[]> {
	...
	for (const directory of directories) {
		...
		try {
			const info = JSON.parse(await fs.readFile(path.join(folder, 'info.json'), 'utf8')) as {
				type?: string;
			};
			...
			if (!messages)
				throw new Error('A conversation could not be read; memory processing will retry.');
