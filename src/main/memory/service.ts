...
export class Memory implements MemoryService {
	...
	async edit(markdown: string, expected?: string): Promise<void> {
		...
		await this.lock(async () => {
			...
			if (expected !== undefined && current !== expected)
				throw new Error('Memory changed while editing. Reload before saving.');
	...
	async clear(): Promise<void> {
		...
		await this.lock(async () =>
			this.replace(
				'',
				parseMemories(await this.read()).map((entry) => entry.id)
			)
		);
	}
	...
	private async replace(markdown: string, suppressed: string[]): Promise<void> {
		...
		const state = {
			...
			checkpoints: Object.fromEntries(
				sources.map((source) => [source.id, source.messages.map((message) => message.fingerprint)])
			),
		...
		try {
			...
			const { mutation: _mutation, ...previous } = this.state;
			this.persist(previous);
			throw error;
		}
		const { mutation: _mutation, ...committed } = state;
	...
	private async initialize(): Promise<void> {
		...
		await this.lock(async () => {
			...
			if (state.mutation) {
				const { mutation, ...previous } = state;
				state =
					fingerprint(await this.read()) === mutation.digest
						? {
								...previous,
								initialized: true,
								checkpoints: mutation.checkpoints,
			...
			if (!state.initialized) {
				...
				state = {
					...
					checkpoints: Object.fromEntries(
						sources.map((source) => [
							source.id,
							source.messages.map((message) => message.fingerprint),
						])
					),
				};
				this.persist(state);
		...
		this.pending = sources.reduce(
			(total, source) =>
				...
				source.messages.filter(
					(message) => !this.state.checkpoints[source.id]?.includes(message.fingerprint)
				).length,
			0
		);
		let batches = 0;
		for (const source of sources) {
			let pending = source.messages.filter(
				(message) => !this.state.checkpoints[source.id]?.includes(message.fingerprint)
			);
				...
				const committed = await this.lock(async () => {
					...
					this.persist({
						...
						checkpoints: {
							...
							[source.id]: source.messages
								.map((message) => message.fingerprint)
								.filter((value) => processed.has(value)),
						},
					});
