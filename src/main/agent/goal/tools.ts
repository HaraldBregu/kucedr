...
export function goalTools(sessionDirectory: string): Tool[] {
	return [
		...
		tool({
			...
			description:
				'Replace the operational steps for the active goal. Do not change its success criteria.',
			inputSchema: z.object({
				steps: z
					.array(
						z.object({
		...
		tool({
			...
			description:
				'Request completion. The runtime rejects completion unless every criterion has recorded evidence.',
			inputSchema: z.object({ summary: z.string().min(1).max(1000) }),
			execute: ({ summary }) => {
				const goal = readGoal(sessionDirectory);
				if (!goal || goal.status !== 'active') throw new Error('No active goal.');
				const unsupported = goal.criteria.filter(
					(item) => !item.satisfied || item.evidenceIds.length === 0
				);
				if (unsupported.length)
					return { completed: false, missingCriteria: unsupported.map((item) => item.id) };
