	...
	for (const snapshot of replacement) {
		...
		try {
			...
			if (!stat.isFile() || stat.size > 2 * 1024 * 1024)
				throw new Error('Invalid file history target.');
			if (before.exists) {
				const selected = identities.at(-1);
				if (
					stat.dev !== selected?.dev ||
					stat.ino !== selected.ino ||
					JSON.stringify(validateFilePath(snapshot.path)) !== JSON.stringify(identities)
				)
				...
				if (createHash('sha256').update(content.subarray(0, offset)).digest('hex') !== before.hash)
					throw new Error(
						'Files changed after this operation; refusing to overwrite newer changes.'
					);
			}
			const restored = Buffer.from(snapshot.content ?? '', 'base64');
			if (restored.length > 2 * 1024 * 1024)
				throw new Error('File history exceeds the byte limit.');
