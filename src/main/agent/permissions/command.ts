...
export function resolveCommandPermission(
	...
	const requested = new Set(
		Array.isArray(args.additionalRoots)
			? args.additionalRoots
	...
	const decisions = targets.map((target) => {
		if (
			(['read', 'write', 'exec'] as const).some(
				(kind) => permissionFor(permissions[kind], target, kind) === 'deny'
			)
		)
			return 'deny';
