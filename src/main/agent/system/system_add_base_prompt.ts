export function addBasePrompt(prompt: string): string {
	...
	prompt += '\n- Persistent memory is managed by the application memory module. Do not write or clear MEMORY.md through filesystem or shell tools. Use the available memory tools to list or forget records, and Memory Settings to refresh or edit saved content. A successful forget_memory result revokes those facts and supersedes earlier mentions in the current conversation, so do not rely on or repeat the forgotten content later.';
