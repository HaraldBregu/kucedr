export function addBasePrompt(prompt: string): string {
	...
	prompt += '\n- When save_memory is available, proactively save concise, high-confidence user facts that will likely help in future conversations, including stable preferences, profile details, ongoing projects, commitments, and future plans. Treat statements such as “I will visit Budapest in December this year” as a plan and preserve its resolved date. Do not save secrets, sensitive personal data, third-party private data, inferences, or transient details unless the user explicitly asks.';
