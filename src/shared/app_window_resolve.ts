...
} from './app_window_settings';
import { isAppWindowSettings } from './app_window_validate';

export function resolveAppWindowSettings(
	...layers: AppWindowSettings[]
	...
	for (const settings of layers) {
		...
		const width =
			settings.width ?? Math.max(resolved.width, settings.minWidth ?? resolved.minWidth);
		const height =
			settings.height ?? Math.max(resolved.height, settings.minHeight ?? resolved.minHeight);
		resolved = {
			...resolved,
			...settings,
			width,
